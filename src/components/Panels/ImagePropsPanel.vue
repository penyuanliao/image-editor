  ...
  <NPanel
    ...
    <div v-if="editorStore.selectedElement" class="properties">
      ...
      <div class="ctrl" style="gap: 4px;">
        ...
        <el-input-number class="el-input" v-model="localX" @change="handleXChange" :controls="false" style="width: 80px;" />
        <span class="label">Y</span>
        <el-input-number class="el-input" v-model="localY" @change="handleYChange" :controls="false" style="width: 80px;" />
      ...
      <div class="ctrl" style="gap: 4px;">
        ...
        <el-input-number class="el-input" v-model="localWidth" @change="handleWidthChange" :controls="false" style="width: 80px;">
          <template #suffix>px</template>
        </el-input-number>
        <span class="label">H</span>
        <el-input-number class="el-input" v-model="localHeight" @change="handleHeightChange" :controls="false" style="width: 80px">
          <template #suffix>px</template>
        </el-input-number>

        <div class="ratio-lock">
          <el-tooltip :content="isRatioLocked ? '解鎖長寬比' : '鎖定長寬比'" placement="top" :auto-close="500">
            <el-button text @click="isRatioLocked = !isRatioLocked" style="width: 32px;">
              <template #icon>
                <el-icon v-if="!isRatioLocked">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 1.56 1.56" xml:space="preserve"><path d="M.543 1.107.825.822.846.801l.252-.252.111-.111.123-.123c.021-.018.024-.045.009-.06L1.287.201C1.275.189 1.254.189 1.236.198L.201 1.227v.003c-.018.018-.021.045-.009.06l.054.054c.015.015.042.012.06-.009l.12-.12c0 .003.117-.108.117-.108M.552.693c-.015 0-.024.012-.03.024Q.518.744.519.771q0 .018.003.036L.636.693z"/><path d="m.249 1.08.12-.12A.2.2 0 0 1 .213.786.194.194 0 0 1 .408.579h.273q.032 0 .063.012L.861.474C.834.459.807.444.78.438A.4.4 0 0 0 .684.423H.423a.35.35 0 0 0-.366.333.34.34 0 0 0 .192.324M1.293.459l-.12.12a.195.195 0 0 1 .162.177.194.194 0 0 1-.195.207H.87A.3.3 0 0 1 .801.951l-.117.117q.04.026.084.039a.3.3 0 0 0 .096.015h.276a.348.348 0 0 0 .153-.663"/><path d="M.903.849h.09c.015 0 .024-.012.03-.024q.004-.027.003-.054 0-.021-.003-.042z"/></svg>
                </el-icon>
                <el-icon v-else>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 1.56 1.56" xml:space="preserve"><path d="m.519 1.083-.033-.042S.465 1.011.459.993A.03.03 0 0 0 .432.978H.414a.197.197 0 0 1-.201-.18.194.194 0 0 1 .195-.207h.273c.036 0 .069.012.096.024a.2.2 0 0 1 .081.093.2.2 0 0 1 .015.078q0 .021-.006.042c-.006.021.009.039.03.036h.102c.015 0 .024-.012.03-.024q.004-.027.003-.054a.37.37 0 0 0-.06-.195.35.35 0 0 0-.288-.156H.42a.354.354 0 0 0-.357.336.35.35 0 0 0 .348.363h.09c.021-.003.033-.033.018-.051m.978-.315A.35.35 0 0 0 1.14.432L1.059.429c-.024 0-.039.03-.024.048a.5.5 0 0 1 .06.09.03.03 0 0 0 .027.015h.018c.105 0 .195.078.201.18a.194.194 0 0 1-.195.207H.873A.24.24 0 0 1 .777.945.2.2 0 0 1 .696.852.2.2 0 0 1 .681.774q0-.021.006-.042C.693.711.678.693.657.696H.555C.54.696.531.708.525.72Q.521.747.522.774a.346.346 0 0 0 .252.333c.03.009.066.015.096.015h.273a.34.34 0 0 0 .354-.354"/></svg>
                </el-icon>
              </template>
            </el-button>
          </el-tooltip>
        ...
        <div class="reset-size">
          <el-tooltip content="還原預設大小" placement="top" :auto-close="500">
            <el-button text style="width: 32px" @click="handleResetSize">
              <template #icon>
                <el-icon>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 1.56 1.56" xml:space="preserve"><path d="M1.395.12h-.09a.046.046 0 0 0-.045.045v.21c0 .027-.015.039-.036.021a.2.2 0 0 0-.03-.03.63.63 0 0 0-.576-.171.6.6 0 0 0-.21.087.64.64 0 0 0-.291.525.62.62 0 0 0 .174.441.62.62 0 0 0 .456.195.64.64 0 0 0 .411-.15.045.045 0 0 0 .003-.066l-.063-.063a.046.046 0 0 0-.06-.003.455.455 0 0 1-.516.042A.455.455 0 0 1 .753.36a.45.45 0 0 1 .354.174c.009.024-.012.036-.039.036h-.21a.046.046 0 0 0-.045.045v.093c0 .024.018.042.042.042h.549a.04.04 0 0 0 .039-.039V.165C1.44.141 1.419.12 1.395.12"/></svg>
                </el-icon>
              </template>
            </el-button>
          </el-tooltip>
