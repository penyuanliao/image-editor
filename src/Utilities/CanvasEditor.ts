...
export class CanvasEditor {
  ...
  public onContextMenu:
    | ((event: IContextMenuEvent, wordPosition: { x: number; y: number }) => void)
    | null = null;
  ...
  public findElementAtPosition(x: number, y: number, skipNotDraggable: boolean = true) {
    ...
    return [...this.store.elements].reverse().find((el) => {
      if (
        (el.type === ElementTypesEnum.Text || el.type === ElementTypesEnum.Image) &&
        el.config.rotation
      ) {
  ...
  private handleContextMenu(event: MouseEvent) {
    ...
    this.onContextMenu?.(
      { x: event.clientX, y: event.clientY, element: clickedElement as ICanvasElement },
      { x, y }
    );
  }
  // 選單位置
  ...
  private handleMouseDown(event: MouseEvent) {
    ...
    // 檢查是否正在剪裁
    const croppingElement = this.store.elements.find(
  ...
  private handleTransformStart(x: number, y: number, action: string, element: ICanvasElement) {
    ...
    if (action === "rot") {
      ...
      this.dragStart.angle =
        Math.atan2(y - element.config.y, x - element.config.x) - this.dragStart.elementRotation;
    } else {
      ...
      if (this.ctx && !["tm", "bm", "ml", "mr"].includes(action)) {
        ...
        if (handles) {
          ...
          this.dragStart.startHandleDistance = Math.hypot(
            startHandlePos.x - element.config.x,
            startHandlePos.y - element.config.y
          );
        }
      }
    }
  ...
  private handleMouseMove(event: MouseEvent) {
    ...
    if (this.isResizing && this.store.selectedElements.length === 1) {
      ...
      if (
        element.type === ElementTypesEnum.Image &&
        this.isResizing &&
        ["tm", "bm", "ml", "mr"].includes(this.isResizing)
      ) {
        // 這邊是計算方向的縮放
        ...
        if (this.isResizing === "tm" || this.isResizing === "bm") {
          ...
          sticker.x =
            this.dragStart.elementX +
            (newHeight / 2 - this.dragStart.elementHeight / 2) * sin * sign;
          sticker.y =
            this.dragStart.elementY +
            (newHeight / 2 - this.dragStart.elementHeight / 2) * cos * sign;
        } else {
          // 'ml' or 'mr'
          // 1. 計算固定邊的中點 (pivot)
          ...
          sticker.x =
            this.dragStart.elementX + (newWidth / 2 - this.dragStart.elementWidth / 2) * cos * sign;
          sticker.y =
            this.dragStart.elementY - (newWidth / 2 - this.dragStart.elementWidth / 2) * sin * sign;
        }
      } else {
        if (
          this.isPivotPointEnabled &&
          (element.type === ElementTypesEnum.Image ||
            element.type === ElementTypesEnum.Text ||
            element.type === ElementTypesEnum.SVG)
        ) {
          ...
          if (element.type === ElementTypesEnum.Image) {
            ...
            (element.config as ITextConfig).fontSize = Math.max(
              10,
              this.dragStart.elementSize * scaleRatio
            );
          } else if (element.type === ElementTypesEnum.SVG) {
            const newWidth = Math.max(10, w * scaleRatio);
            const newHeight = newWidth / this.dragStart.aspectRatio;
          ...
          element.config.x =
            pivotX - pivotLocalX * scaleRatio * cos + pivotLocalY * scaleRatio * sin;
          element.config.y =
            pivotY - pivotLocalX * scaleRatio * sin - pivotLocalY * scaleRatio * cos;
        } else {
          ...
          if (element.type === ElementTypesEnum.Image) {
            (element.config as IImageConfig).width = Math.max(
              10,
              this.dragStart.elementWidth * scaleRatio
            );
            (element.config as IImageConfig).height =
              (element.config as IImageConfig).width! / this.dragStart.aspectRatio;
          } else if (element.type === ElementTypesEnum.Text) {
            (element.config as ITextConfig).fontSize = Math.max(
              10,
              this.dragStart.elementSize * scaleRatio
  ...
  private handleDoubleClick(event: MouseEvent) {
    ...
    if (clickedElement && clickedElement.type === ElementTypesEnum.Text) {
      ...
      this.imageCropEditEnabled &&
      clickedElement &&
      clickedElement.type === ElementTypesEnum.Image
    ) {
  ...
  private clearEditing(clickedElement?: ICanvasElement) {
    const currentlyCroppingElement = this.store.elements.find(
      (el) => (el.config as IImageConfig).cropConfig?.isCropping
    );
  ...
  public updateViewportSize(width: number, height: number, color: string = "transparent") {
    ...
    const constrained = calculateConstrainedSize(
      width,
      height,
      generalDefaults.viewport.maxWidth,
      generalDefaults.viewport.maxHeight
    );
  ...
  public disableCopyAndPasteSupport() {
    if (this.handlePaste) document.removeEventListener("paste", this.handlePaste);
    if (this.handleCopy) document.removeEventListener("copy", this.handleCopy);
  }
  // 對齊物件
  public align(
    horizontally: "left" | "center" | "right" | string | null,
    vertically: "top" | "middle" | "bottom" | string | null
  ) {
    const selectedElements = this.store.selectedElements;
